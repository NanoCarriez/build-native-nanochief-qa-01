...
function isolatedHeadCtx(extra = {}) {
  return { cwd: mkdtempSync(join(tmpdir(), "grok-pwa-isolated-")), ...extra };
}
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, isolatedHeadCtx({ appName: "Wild Race" }));
...
test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", isolatedHeadCtx({
    host: "wild-race.grok.me",
  }));
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", isolatedHeadCtx({ appName: "Solo" }));
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector(isolatedHeadCtx({ appName: "Wild Race" }));
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", isolatedHeadCtx({ appName: "Wild Race" }));
